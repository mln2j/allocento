...
export class DashboardPage implements OnInit {
  ...
  private async loadFromCache() {
    try {
      ...
      if (dashboardCache) {
        currentBalance = dashboardCache.total_balance ?? 0;
        recentTxs = dashboardCache.recent_transactions ?? [];
        primaryAccount = dashboardCache.primary_account ?? null;
        spendingStats = dashboardCache.spending_stats ?? [];
        dailySpending = dashboardCache.daily_spending ?? [];
        workspaceName = dashboardCache.workspace_name ?? '';
      }

      // Uvijek dodaj offline transakcije koje još nisu sinkronizirane, čak i ako nema cachea
      try {
        const offlineTxs = await this.localDb.getAll('transactions');
        const queuedTxs = offlineTxs.filter(t => t.id < 0).sort((a, b) => new Date(b.date).getTime() - new Date(a.date).getTime());
      ...
      this.totalBalance.set(currentBalance);
      this.primaryAccount.set(primaryAccount);
      this.recentTransactions.set(recentTxs);
      this.spendingStats.set(spendingStats);
      this.dailySpending.set(dailySpending);
      this.setDefaultTab();
      this.activeWorkspaceName.set(workspaceName);
